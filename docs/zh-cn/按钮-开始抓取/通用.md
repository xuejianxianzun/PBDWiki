...
![](../images/20260610_005351.png)

这个页数需要结合“间隔时间”考虑。假如你想每隔 30 分钟进行一次抓取，并且该页面在 30 分钟的新作品数量不会超过 1 页，那么就可以设置为 `1`。

3. 点击 `定时抓取` 按钮，下载器会显示一个输入框，让你设置间隔时间：

<div class="XZInputWrap" id="input1757240375255"><p class="XZInputInstruction">定时抓取的间隔时间 (分钟)</p><div class="XZInputContainer"><input class="XZInput w100" placeholder="" type="text" value="30"><button class="XZInputButton hasRippleAnimation">
      <span>提交</span>
...
<span class="log" style="color: rgb(0, 189, 23);">定时抓取已启动，间隔时间：30 分钟。<br>如果你想修改间隔时间，可以在“更多”选项卡里修改设置：定时抓取的间隔时间。<br></span>

**提示：**

- 下载器不会立刻开始抓取，而是等到设定的时间之后才会开始第一次抓取。
- 推荐启用“更多”选项卡里的 [不下载重复文件](/zh-cn/设置-下载/下载记录?id=不下载重复文件) 功能，以避免不必要的重复下载。
...
3. 建议启用 [不下载重复文件](/zh-cn/设置-下载/下载记录?id=不下载重复文件) 功能，以避免下载重复的文件。
...
7. 定时抓取总是会自动开始下载。
8. 在点击 `定时抓取` 按钮时，下载器会让你设置每次抓取的间隔时间。这个时间会与 [定时抓取的间隔时间](/zh-cn/设置-抓取/执行策略?id=定时抓取的间隔时间) 设置保持一致。
...
当你点击 `手动选择作品` 按钮之后，这个按钮上的文字会发生变化，根据状态显示“暂停选择”、“继续选择”。此外，这个按钮后面还会显示下面两个按钮。
