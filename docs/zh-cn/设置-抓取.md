...
在抓取时，如果有作品因为不满足过滤条件而被排除，下载器会在日志里显示是哪些设置导致的。例如：

<div class="logContent beautify_scrollbar">
<span class="log" style="color: rgb(210, 126, 0);">下载器排除了一些作品，原因：作品类型</span>
<br>
<span class="log" style="color: rgb(210, 126, 0);">下载器排除了一些作品，原因：图片的宽高</span>
<br>
<span class="log" style="color: rgb(210, 126, 0);">下载器排除了一些作品，原因：收藏数量</span>
</div>
...
1. 设置抓取的页数为 100，然后从第 1 页开始抓取。下载器会抓取第 1 - 100 页里的作品。
2. 下载完所有文件之后，你可以关闭该页面，并重新打开，以彻底释放内存。
3. 你可以等待一段时间（例如等到第二天）再继续抓取。
4. 跳转到第 101 页，开始下一次抓取。下载器会抓取第 101 - 200 页里的作品。
5. 以此类推。

**跳转页码的技巧：**

在有页码的页面里，你可以修改地址栏里的页码，快速进入指定页面。下面是一个例子。

当你进入搜索页面时，默认会进入第 1 页，此时地址栏里没有页码。

随便点击一个页码，例如第 2 页：

![](images/20250907_194850.png)

进入第 2 页之后，地址栏里的 `p=2` 就是页码：

![](images/20250907_194909.png)

把 `2` 改为你需要的数字，例如 `101`，然后回车：
...
<p class="option" data-no="96" style="display: flex;">
      <a href="https://xuejianxianzun.github.io/PBDWiki/#/zh-cn/设置-抓取?flag=96" target="_blank" class="settingNameStyle">
      ...
      <button class="gray1 textButton" id="showLooseMatchOriginalTip" type="button" data-xztext="_帮助">帮助</button>
    </p>
...
<p class="option" data-no="3" style="display: flex;">
    ...
    <span data-xztext="_多图作品只下载前几张图片">多图作品只抓取<span class="key">前几张</span>图片</span>
    <span class="gray1"> ? </span>
    </span>
...
<p class="option" data-no="99" style="display: flex;">
   <a href="https://xuejianxianzun.github.io/PBDWiki/#/zh-cn/设置-抓取?flag=99" target="_blank" class="has_tip settingNameStyle" data-xztip="_不抓取下载过的作品的说明" data-tip="如果下载器里有这个作品的下载记录，就不会抓取它。">
...
**注意：** 该功能依赖下载器保存的下载记录，所以不是完全准确的。如果你清除了下载器的下载记录，那么下载器会忘记它下载过哪些文件。如果你想详细了解下载器的下载记录，可以查看 [管理下载记录](/zh-cn/设置-更多-下载?id=管理下载记录) 里的说明。
...
<p class="option" data-no="8" style="display: flex;">
    ...
    <span class="has_tip settingNameStyle" data-xztip="_宽高比的提示" data-tip="宽高比是宽度除以高度得到的数字。&lt;br&gt;宽高比小于 1 时，图片是竖图。&lt;br&gt;宽高比大于 1 时，图片是横图。&lt;br&gt;宽高比越大，图片越扁长。">
      <label for="ratio3" style="padding: 0;" data-xztext="_宽高比" class="active">宽高比</label>
      <span class="gray1"> ? </span>
    </span>
...
<p class="option" data-no="9" style="display: flex;">
      <a href="https://xuejianxianzun.github.io/PBDWiki/#/zh-cn/设置-抓取?flag=9" target="_blank" class="has_tip settingNameStyle" data-xztip="_设置id范围提示" data-tip="您可以输入一个作品 ID，抓取 ID 比它大的作品（新作品）或者比它小的作品（旧作品）">
...
这个设置有助于进行增量更新。因为作品的 ID 和发布时间是正相关的：发布越晚的作品，ID 就越大。
...
<p class="option" data-no="10" style="display: flex;">
      <a href="https://xuejianxianzun.github.io/PBDWiki/#/zh-cn/设置-抓取?flag=10" target="_blank" class="has_tip settingNameStyle" data-xztip="_设置投稿时间提示" data-tip="您可以下载指定时间内发布的作品">
      ...
      <span class="subOptionWrap" data-show="postDate" style="display: none;">
        ...
        <button class="textButton grayButton mr0" type="button" role="setDate" data-for="postDateStart" data-value="2009-01-01T00:00" data-xztext="_过去">过去</button>
        <button class="textButton grayButton" type="button" role="setDate" data-for="postDateStart" data-value="now" data-xztext="_现在">现在</button>
        -&nbsp;
        ...
        <button class="textButton grayButton mr0" type="button" role="setDate" data-for="postDateEnd" data-value="now" data-xztext="_现在">现在</button>
        <button class="textButton grayButton" type="button" role="setDate" data-for="postDateEnd" data-value="2100-01-01T00:00" data-xztext="_未来">未来</button>
      </span>
    </p>
...
例如，我要下载 2010 年之前的所有作品，可以把起始时间设置为早于 pixiv 创建日期的时间（你也可以点击 `过去` 按钮），结束时间设置为 2010 年，例如：

`2009-01-01 00:00 - 2010-01-01 00:00`

如果我要下载 2020 年之后的所有作品，可以把结束时间设置为未来的时间（你也可以点击 `未来` 按钮），例如：
...
<p class="option" data-no="94" style="display: flex;">
      <a href="https://xuejianxianzun.github.io/PBDWiki/#/zh-cn/设置-抓取?flag=94" target="_blank" class="has_tip settingNameStyle" data-xztip="_标题必须含有的说明" data-tip="你可以要求作品的标题里必须含有特定字符。不区分大小写。&lt;br&gt;
...
<p class="option" data-no="95" style="display: flex;">
      <a href="https://xuejianxianzun.github.io/PBDWiki/#/zh-cn/设置-抓取?flag=95" target="_blank" class="has_tip settingNameStyle" data-xztip="_标题不能含有的说明" data-tip="你可以要求作品的标题里不能含有特定字符。不区分大小写。&lt;br&gt;
你可以设置多条字符，每条之间使用逗号(,)分割。&lt;br&gt;
...
排除的优先级大于包含的优先级。">
      ...
      <span class="subOptionWrap" data-show="titleExcludeSwitch" style="display: none;">
        ...
        <label for="alsoCheckSeriesTitle" class="has_tip active" data-xztext="_也检查系列标题" data-xztip="_也检查系列标题的说明" data-tip="如果作品属于一个系列，启用此设置可以同时检查系列标题是否符合条件。">也检查系列标题</label>
...
**子选项：**
- `也检查系列标题`：启用此设置后，下载器会同时检查系列标题。如果系列标题里含有被排除的字符，那么下载器就不会抓取这个系列，也不会抓取它里面的单个作品。
