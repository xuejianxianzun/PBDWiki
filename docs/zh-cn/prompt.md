...
### 在终端的 CLI 里
- **批量处理更改模式**：我可能会让你把中文 md 文件里做的修改同步到对应的英语 md 文件里。你需要从对应的英语 md 文件里删除被移除的内容，并把新增/有变化的内容翻译后添加到对应位置。注意：1. 在这种情况里，如果发生更改的文件不在前面列出的中文文件列表里，就不需要翻译它。2. 一个段落里可能只有部分文字被修改，但是在该模式里翻译时，应该翻译修改的内容所在的整段文字，而不是只翻译发生变化的部分文字。如果只翻译发生变化的部分文字，会导致这个段落的英语翻译存在错误。
- **翻译指定文件模式**：我可能会让你翻译指定文件，此时需要翻译它的完整内容并保存到对应的英语文件里。

----------

**注意：** 
- 在上述所有方式里，都需要遵循“需要翻译哪些内容”部分的要求。也就是说，即使我让你批量处理更改或者翻译整个文件，里面可能依然有些内容是不需要翻译的。