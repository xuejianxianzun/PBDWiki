...
有些用户想让 `R-18` 和 `R-18G` 作品分别保存到**两个**文件夹里，此时可以在这个设置里输入 `R-18,R-18G`。当作品含有 `R-18` 标签时就会建立 `R-18` 文件夹；如果含有 `R-18G` 标签，就会建立 `R-18G` 文件夹。
...
- 你可以修改这里的目录名；可以使用命名规则里的标记。
- R-18 和 R-18G 作品都会存放到这里设置的文件夹里，不能分开存放。
- 你可以使用命名标记 `{age_r}` 来代替这个设置。 `{age_r}` 可以单独输出 `R-18` 或 `R-18G`，也可以放置在命名规则里的任意位置，更加灵活。
