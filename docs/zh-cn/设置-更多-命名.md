...
- `文件夹规则`：为多图作品添加的文件夹的名称。你可以像设置命名规则一样使用各种标记和自定义文字，也可以使用 `/` 来建立文件夹。默认值是 `{id_num}`，也就是使用作品 ID（不带序号）建立文件夹，把文件存放在里面。
...
- 如果你要在文件夹名字里使用作品 ID，那么不应该使用 `{id}` 标记，而是应该使用 `{id_num}`。因为多图作品里的每张图片的 `{id}` 都不同，如果使用 `{id}` 会导致每个图片都会建立一个文件夹。
...
?> 命名标记 `{p_num}` 也会输出序号，它输出的是纯数字的 `0`、`1`、`2`。这个设置对 `{p_num}` 也会生效，即可以去掉 `{p_num}` 输出的 `0`。
