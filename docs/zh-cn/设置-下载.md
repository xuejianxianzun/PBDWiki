...
`{id}` 带有序号，例如 `44920385_p0`。如果你想移除 `_p`，可以把 `{id}` 替换为 `{id_num} {p_num}`，这样生成的字符是 `44920385 0`。

?>注意：如果你想替换 `{id}`，那么命名规则里必须同时含有 `{id_num}` 和 `{p_num}`，这是为了防止文件名重复。
...
<div class="option new" data-no="64" style="display: flex;"><button type="button" class="pinButton" data-title="_置顶">📌</button>
      ...
      <span class="subOptionWrap noGrow" data-show="noFolderSwitch" style="display: none;">
        ...
        <label for="noFolderWhenSingleImageWork" data-xztext="_单图作品" class="active">单图作品</label>
        ...
        <span class="beautify_checkbox" tabindex="0"></span>
        <label for="noFolderWhenMultiImageWork" data-xztext="_多图作品">多图作品</label>
      ...
      <button type="button" class="gray1 textButton showMsgBtn" data-title="_不创建文件夹" data-msg="_以下情况不创建文件夹的帮助内容" data-xztext="_帮助">帮助</button>
    </div>

如果你设置的命名规则会建立文件夹，同时你又希望部分作品不要建立文件夹，而是直接保存到下载目录里，可以启用这个设置。
