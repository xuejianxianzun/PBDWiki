...
我建议你选择 EPUB 格式，因为 TXT 格式存在一些缺点：
...
<p class="option new" data-no="91" style="display: flex;">
      <a href="https://xuejianxianzun.github.io/PBDWiki/#/zh-cn/设置-更多-下载?flag=91" target="_blank" class="settingNameStyle" data-xztext="_合并系列小说时的命名规则">合并系列小说时的<span class="key">命名</span>规则</a>
      <span class="rowWrap">
        <textarea class="centerPanelTextArea beautify_scrollbar" name="seriesNovelNameRule" rows="1"></textarea>
        <button class="showFileNameTip textButton" id="showSeriesNovelNameTip" type="button" data-xztext="_提示">提示</button>
      </span>
    </p>

你可以设置下载器合并系列小说时生成的合集文件的名字。

你可以在下载器的设置里点击“提示”按钮查看详细说明，此处不再重复。

默认的规则是 `novel series/{page_tag}/{series_title}-{series_id}-{user}-{part}-{tags}.{ext}`。
