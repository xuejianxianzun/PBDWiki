...
你可以选择把小说保存为 TXT 或者 EPUB 格式。

下载器默认的保存格式是 TXT，因为它更通用，而 EPUB 需要使用小说阅读器打开，比 TXT 麻烦一些。还有些用户甚至不知道 EPUB 文件是什么。但我建议你选择 EPUB 格式。

**建议使用 EPUB 格式：**

下载器可以把小说保存成 TXT 或 EPUB 格式。TXT 格式存在一些缺点：
- TXT 文件的内容是纯文字，不能显示富文本样式（例如文字样式、超链接等）。
- TXT 文件的内容是纯文字，当含有多篇小说（多个章节）时，很多阅读器都不能正确识别章节格式。
- TXT 文件无法保存小说里的插画，所以只能把图片保存成单独的文件。有时一个 TXT 文件可能有十几个相关图片，非常杂乱。

EPUB 是电子书的格式，可以显示富文本样式，支持章节目录，并且可以在内部保存封面图片和插图。

?> 请使用小说阅读器查看 EPUB 小说。有些软件虽然能打开 EPUB 文件，但可能存在兼容性问题，例如 WPS。
