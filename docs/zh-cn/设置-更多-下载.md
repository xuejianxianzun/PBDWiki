...
小说的元数据也大致相同，但是没有 `Original` 和 `Size`。此外，小说的 `Thumbnail` 就是它的封面图片的 URL。
...
?> 下载动图时，前 3 个选项是相同的，因为动图没有对应尺寸的图片，所以下载器都会下载动图的源文件。“方形缩略图”对动图有效。

--------------

有时候也许你不需要下载原图，只是想要查看作品的缩略图，就可以选择更小的尺寸。

还有些其他的使用场景：有时你可能想批量收藏一些作品，但是并不需要下载它们。你可以下载这些作品，并启用“下载之后收藏作品”功能。

如果使用“原图”尺寸，下载这些作品就会比较耗费时间，此时可以使用“方形缩略图”来加快下载速度。

## 文件体积限制

![](./images/20220802_204758.png)

默认未启用。

当一个文件开始下载时，如果它的体积不在指定的范围内，下载器就会跳过下载这个文件。

?>这里可以输入带小数点的数字，例如 0.5 MiB。

## 文件下载顺序

![](./images/20230131_140354.png)

默认未启用。

这不是必须的设置；如果你有需要，可以启用此设置。

以前，下载器把文件保存到硬盘上时，文件的保存顺序（修改时间）通常是混乱的，不能用修改时间来排序。

如果你需要**使用文件的修改时间来排序**，你可以启用这个设置，并选择你需要的下载顺序。下载器会按照你设置的顺序把文件保存到硬盘上。
...
即使你没有开启这个功能，下载器也会保存下载记录，以备之后使用。

在下载一个文件之前，下载器会查询记录，判断这个文件是不是重复文件。

**注意：**

!>如果你清除了浏览器数据中的“Cookie 及其他网站数据”，那么下载器保存的数据也被清空。你可以在此操作之前先导出下载记录。

此外，下载器只能查询自己的下载记录，不会检查硬盘上的文件（因为没有这个权限）。所以这个方法并不完美。
...
!>如果你清除了浏览器数据中的“Cookie 及其他网站数据”，那么下载器保存的数据也被清空。你可以先导出下载记录，以便之后进行恢复。
...
启用之后，当浏览器下载文件时，不会在窗口底部显示下载栏。

不仅本程序下载文件时不会显示下载栏，通过其他途径创建的下载任务也同样不会显示下载栏。
